...
- **Variable Expansion Modifier:** 
	- ``${VAR:-default}``: User ``default`` if ``VAR`` is unset or null. Useful for providing fallback values.
...
echo "Password entered (for demonstrating): $password"
```
...
if [[ "$num" -gt 0 && "$num" -lt 10 ]]; then
	...
	echo "Number if either 100 or greater, or less then 0."
else
...
if [[ "$num" -gt 0 ]]; then
	echo "$sum is greater than zero (positive)."
elif [[ "$num" -lt 0 ]]; then
...
#!/bin/bash
num=5
if [ "$num" -gt 0 ]; then # Note the spaces around the brackets are operators
	echo "Positive (using [ ])"
fi
```
...
- **Implementing Complex Workflows:** ``case`` statements are excellent for building interactive scripts or handling different execution paths bassed on command-line arguments.
---
